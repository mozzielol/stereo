...
class BNNLayer(snt.AbstractModule):
    ...
    def __init__(self, n_inputs, n_outputs, init_mu=0.0, init_rho=0.0, name="BNNLayer"):
        ...
        #self.w_rho = tf.Variable(tf.log(1.0 + tf.exp(init_rho*tf.ones([self.n_inputs, self.n_outputs]))))#tf.Variable(tf.random_normal([self.n_inputs, self.n_outputs], -4, 0.1)) ## -4 or even -3 are good initializations
        #self.w_sigma = tf.log(1.0 + tf.exp(self.w_rho))
        self.w_sigma = tf.Variable(tf.log(1.0 + tf.exp(init_rho*tf.ones([self.n_inputs, self.n_outputs]))))
        ...
        self.b_sigma = tf.Variable(tf.log(1.0 + tf.exp(init_rho*tf.ones([self.n_outputs]))))
